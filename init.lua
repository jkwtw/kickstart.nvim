...
if vim.loop.os_uname().sysname == 'Windows_NT' then
  ...
  vim.api.nvim_create_autocmd('ExitPre', {
    callback = function()
      local shell_name = vim.fs.basename(os.getenv 'SHELL' or os.getenv 'COMSPEC' or 'shell')

      io.write(string.format('\027]2;%s\007', shell_name))
      io.flush()
    end,
  })
end
...
require('lazy').setup({
  ...
  { -- Highlight, edit, and navigate code
    ...
    opts = {
      ensure_installed = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'query', 'vim', 'vimdoc', 'doxygen', 'cmake', 'ini' },
      auto_install = true,
      highlight = {
        enable = true,
  ...
  },
  {
    ...
    ft = { 'plantuml', 'pu', 'puml' }, -- load only for .plantuml or .pu files
  },
